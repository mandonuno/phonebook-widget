...
class Database:
    ...
    def __init__(self, db):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS phonebook (id INTEGER PRIMARY KEY, first text, last text, cell integer, email text)")
        self.conn.commit()

    def insert(self, first, last, cell, email):
        """Inserts data into contacts db"""
        self.cur.execute("INSERT INTO phonebook VALUES (NULL,?,?,?,?)", (first,last,cell,email))
        self.conn.commit()
    ...
    def search(self, first="", last="", cell="", email=""):
        ...
        self.cur.execute("SELECT * FROM phonebook WHERE first=? OR last=? OR cell=? OR email=?", (first,last,cell,email))
        rows = self.cur.fetchall()
        return rows
    ...
    def update(self, id, first, last, cell, email):
        ...
        self.cur.execute("UPDATE phonebook SET first=?, last=?, cell=?, email=? WHERE id=?", (first,last,cell,email,id))
        self.conn.commit()
